    ...
    for (int i = 0; i < node_list.size(); ++i) {
        ...
        if(i + 1 == node_list.size()) {
            ...
            if(((node1.index >> 1) == (node2.index >> 1)) && (node1.level == node2.level)) {
                ...
                // printf("node1.index >> 1 : %ld , node1.level - 1 : %d",node1.index >> 1,node1.level - 1);
