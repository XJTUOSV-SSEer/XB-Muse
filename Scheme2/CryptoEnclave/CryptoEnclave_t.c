...
static sgx_status_t SGX_CDECL sgx_ecall_insert_set_string(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_insert_set_string_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_insert_set_string_t* ms = SGX_CAST(ms_ecall_insert_set_string_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	void* _tmp_DelInd_ptr = ms->ms_DelInd_ptr;
	size_t _tmp_DelInd_ptr_size = ms->ms_DelInd_ptr_size;
	size_t _len_DelInd_ptr = _tmp_DelInd_ptr_size;
	void* _in_DelInd_ptr = NULL;
	char* _tmp_val_tag = ms->ms_val_tag;
	size_t _tmp_val_tag_size = ms->ms_val_tag_size;
	size_t _len_val_tag = _tmp_val_tag_size;
	char* _in_val_tag = NULL;

	CHECK_UNIQUE_POINTER(_tmp_DelInd_ptr, _len_DelInd_ptr);
	CHECK_UNIQUE_POINTER(_tmp_val_tag, _len_val_tag);
	...
	if (_tmp_DelInd_ptr != NULL && _len_DelInd_ptr != 0) {
		_in_DelInd_ptr = (void*)malloc(_len_DelInd_ptr);
		if (_in_DelInd_ptr == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_DelInd_ptr, _len_DelInd_ptr, _tmp_DelInd_ptr, _len_DelInd_ptr)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ecall_insert_set_string(_in_DelInd_ptr, _in_val_tag, _tmp_DelInd_ptr_size, _tmp_val_tag_size);
	if (_in_DelInd_ptr) {
		if (memcpy_s(_tmp_DelInd_ptr, _len_DelInd_ptr, _in_DelInd_ptr, _len_DelInd_ptr)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_DelInd_ptr) free(_in_DelInd_ptr);
	if (_in_val_tag) free(_in_val_tag);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_insert_set_string, 0, 0},
	}
};
